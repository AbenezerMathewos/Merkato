...
const COUPONS_DATABASE = {
    ...
    'HABESHA15':   { discountType: 'percentage', value: 15, minOrder: 1500, maxDiscount: 600, description: '15% Habesha specialty discount' },
    'ETHIOPIA':    { discountType: 'percentage', value: 15, minOrder: 2000, maxDiscount: 800, description: '15% national pride discount' },
    'FREESHIP':    { discountType: 'shipping',   value: 100, minOrder: 500,  maxDiscount: 300, description: 'Free Express Shipping' },
...
router.post('/validate', (req, res) => {
    ...
    if (!code) return res.status(400).json({ success: false, valid: false, message: 'Please provide a coupon code' });

    const normalized = code.trim().toUpperCase();
    const coupon = COUPONS_DATABASE[normalized];

    if (!coupon) {
        return res.status(404).json({ success: false, valid: false, message: 'Invalid or expired coupon code' });
    }
    ...
    res.json({
        success: true,
        valid: true,
        message: `🎉 Promo code "${normalized}" applied!`,
        code: normalized,
